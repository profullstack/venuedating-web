import { xlsxToMarkdownService } from '../services/xlsx-to-markdown-service.js';
import { storageService } from '../services/storage-service.js';
import { validateBody, validators } from '../middleware/error-handler.js';
import { errorUtils } from '../utils/error-utils.js';

/**
 * Route handler for converting XLSX to Markdown
 * @param {Object} c - Hono context
 * @returns {Response} - Markdown content response
 */
export async function xlsxToMarkdownHandler(c) {
  try {
    ...
    // Convert XLSX to Markdown using pandoc
    const markdownContent = await xlsxToMarkdownService.convertToMarkdown(fileBuffer);
...
/**
 * Route configuration for XLSX to Markdown endpoint
 */
export const xlsxToMarkdownRoute = {
  method: 'POST',
  path: '/api/1/xlsx-to-markdown',
  middleware: [validateBody(validators.xlsxFileContent)],
  handler: xlsxToMarkdownHandler
};