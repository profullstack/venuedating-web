import { pptxToMarkdownService } from '../services/pptx-to-markdown-service.js';
import { storageService } from '../services/storage-service.js';
import { validateBody, validators } from '../middleware/error-handler.js';
import { errorUtils } from '../utils/error-utils.js';

/**
 * Route handler for converting PPTX to Markdown
 * @param {Object} c - Hono context
 * @returns {Response} - Markdown content response
 */
export async function pptxToMarkdownHandler(c) {
  try {
    ...
    // Convert PPTX to Markdown using pandoc
    const markdownContent = await pptxToMarkdownService.convertToMarkdown(fileBuffer);
    ...
    console.error('Error in PPTX to Markdown conversion:', error);
    return errorUtils.handleError(error, c);
  }
}

/**
 * Route configuration for PPTX to Markdown endpoint
 */
export const pptxToMarkdownRoute = {
  method: 'POST',
  path: '/api/1/pptx-to-markdown',
  middleware: [validateBody(validators.pptxFileContent)],
  handler: pptxToMarkdownHandler
};