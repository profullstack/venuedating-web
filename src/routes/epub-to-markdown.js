import { epubToMarkdownService } from '../services/epub-to-markdown-service.js';
import { storageService } from '../services/storage-service.js';
import { validateBody, validators } from '../middleware/error-handler.js';
import { errorUtils } from '../utils/error-utils.js';

/**
 * Route handler for converting EPUB to Markdown
 * @param {Object} c - Hono context
 * @returns {Response} - Markdown content response
 */
export async function epubToMarkdownHandler(c) {
  try {
    ...
    const markdownContent = await epubToMarkdownService.convertToMarkdown(fileBuffer);
    ...
    console.error('Error in EPUB to Markdown conversion:', error);
    return errorUtils.handleError(error, c);
  }
}

/**
 * Route configuration for EPUB to Markdown endpoint
 */
export const epubToMarkdownRoute = {
  method: 'POST',
  path: '/api/1/epub-to-markdown',
  middleware: [validateBody(validators.epubFileContent)],
  handler: epubToMarkdownHandler
};