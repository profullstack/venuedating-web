import { docxToMarkdownService } from '../services/docx-to-markdown-service.js';
import { storageService } from '../services/storage-service.js';
import { validateBody, validators } from '../middleware/error-handler.js';
import { errorUtils } from '../utils/error-utils.js';

/**
 * Route handler for converting DOCX to Markdown
 * @param {Object} c - Hono context
 * @returns {Response} - Markdown content response
 */
export async function docxToMarkdownHandler(c) {
  try {
    ...
    const markdownContent = await docxToMarkdownService.convertToMarkdown(fileBuffer);
...
/**
 * Route configuration for DOCX to Markdown endpoint
 */
export const docxToMarkdownRoute = {
  method: 'POST',
  path: '/api/1/docx-to-markdown',
  middleware: [validateBody(validators.docxFileContent)],
  handler: docxToMarkdownHandler
};